  ...
  useEffect(() => {
    const checkCollision = () => {
      if (beamRef.current && parentRef.current && !cycleCollisionDetected) {
        ...
        const viewportBottom = typeof window !== "undefined" ? window.innerHeight : parentRect.bottom;

        if (beamRect.bottom >= viewportBottom) {
          const relativeX =
            beamRect.left - parentRect.left + beamRect.width / 2;
          const relativeY = viewportBottom - parentRect.top;
  ...
  return (
    <>
      <motion.div
        ...
        style={{
          background: `linear-gradient(to top, var(--accent-purple), var(--accent-yellow), transparent)`,
        }}
      />
      <AnimatePresence>
...
const Explosion = (props: React.HTMLProps<HTMLDivElement>) => {
  ...
  return (
    <div {...rest} className={cn("absolute z-50 h-2 w-2", className)}>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        ...
        style={{
          background: `linear-gradient(to right, transparent, var(--accent-yellow), var(--accent-purple), transparent)`,
        }}
      />
      {spans.map((span) => (
        <motion.span
